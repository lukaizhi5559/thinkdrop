...
app.whenReady().then(async () => {
    ...
    async function runSkillHealthCheck() {
      ...
      try {
        ...
        for (const skillName of skillDirs) {
          ...
          // Skip browser automation skills generated by explore.agent — they require
          // a live browser session + playwright internals that don't exist in the
          // isolated probe child process. Probing them crashes the app (Cannot find module).
          try {
            const skillJsonPath = path.join(skillsBase, skillName, 'skill.json');
            if (fs.existsSync(skillJsonPath)) {
              const skillMeta = JSON.parse(fs.readFileSync(skillJsonPath, 'utf8'));
              if (skillMeta.source_domain || skillMeta.sourceDomain || skillMeta.source_action || skillMeta.sourceAction) {
                console.log(`[SkillCron] ${skillName} → skipped (browser automation skill)`);
                continue;
              }
            }
          } catch (_) { /* non-fatal — probe anyway if skill.json unreadable */ }
  ...
  ipcMain.on('prompt-queue:submit', (_event, { prompt, selectedText = '', responseLanguage = null, isAskUserAnswer = false } = {}) => {
    ...
    if (pendingPlanContext) {
      ...
      if (planSessionId !== currentSessionId) {
        ...
        // We need to defer this check until after StateGraph execution
        ...
        enqueueOpts._planSessionId = planSessionId; // Store for later validation
        console.log(`[Plan] Plan correction mode enqueue: basePlan=${pendingPlanContext.planFile || 'none'} (session: ${currentSessionId})`);
  ...
  async function runPromptThroughStateGraph(prompt, { selectedText = '', sessionId = null, userId = 'default_user', responseLanguage = null, promptQueueId = null, _planFile = null, _forceNewPlan = false, _skillPlan = null, _skillPlanFile = null, _planCorrectionMode = false, _planCorrectionText = null, _basePlanFile = null, _skillPlanJson = null, _planCorrectionSourcePrompt = null } = {}) {
    ...
    try {
      ...
      // Clear pending plan context when a new session was created to prevent plan correction mode
      if (finalState._newSessionCreated) {
        console.log('[Plan] New session created - clearing pending plan context to prevent correction mode');
        pendingPlanContext = null;
        if (resultsWindow && !resultsWindow.isDestroyed()) {
          safeSend(resultsWindow, 'automation:progress', { type: 'plan:mode:cleared', source: 'new-session' });
        }
        if (promptCaptureWindow && !promptCaptureWindow.isDestroyed()) {
          safeSend(promptCaptureWindow, 'automation:progress', { type: 'plan:mode:cleared', source: 'new-session' });
        }
      }
  ...
  ipcMain.on('skills:list', async () => {
    try {
      ...
      const items = await Promise.all(allRows.map(async (row) => {
        ...
        // contractMd is now included in skill.list response — no need for a per-skill skill.get call
        const cm = (row.source === 'local' || isScout) ? '' : (row.contractMd || '');
        const fmMatch = cm.match(/^---\s*\n([\s\S]*?)\n---/);
        const fm = fmMatch ? fmMatch[1] : '';
        const triggerMatch  = fm.match(/^trigger\s*:\s*(.+)$/m);
        const scheduleMatch = fm.match(/^schedule\s*:\s*(.+)$/m);
